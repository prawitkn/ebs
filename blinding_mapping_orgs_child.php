...
<body>
	...
	<?php
		$result = mysqli_query($db,"select * from rtarfwen_m_buildings where code='".$user_building_code."' ");
		$r='';
		if($result){
...
<h3>กำหนดหน่วยเข้าเวร <?php echo($user_building_code==0 ? 'บก.ทท.' : $r['name'] ); ?></h3>
 <form action = "blinding_mapping_orgs_child.php" method = "post" class="form-horizontal">
 

 <?php echo '<input type="hidden" name="building_code" id="hid_building_code" value="'. ($user_building_code==0 ? '1' : $user_building_code ) .'">'; ?>
...
<form method="post" action="blinding_mapping_orgs_submit.php">
	<?php
		if(isset($_POST['year_month']) && !empty($_POST['year_month']) && isset($_POST['building_code']) && !empty($_POST['building_code']) ){			
		   ...
		   $ses_sql = mysqli_query($db,"select * from rtarfwen_t_duty_headers where year_month_code='$year_month_code' and building_code='$building_code' ");
		   ...
		   if($rowcount>0){
			   ...
			   echo '<table class="table" id="tbl_main">';
			   echo '<thead>
			   ...
			   $sql = "
						...
						and x.building_code='$building_code'
						order by y.date asc
						";
			   ...
			   while($r = mysqli_fetch_array($ses_sql)) {
				  ...
				  echo '	<td>
					...
					echo '	<td>
								<select name="org_code5-'. (string)$icount .'">';
									echo '<option value=""> -- เลือก --</option>';
								while($x = mysqli_fetch_array($orgs)) {
								  echo '<option value="'.$x['code'].'" ';
									if($x['code'] == $r['org_code5']){ echo ' selected="selected" '; }
								  echo '>'.$x['name'].'</option>';
								}
								mysqli_data_seek($orgs,0);
					echo '		</select>
					...
					echo '	<td>
								<select name="org_code6-'. (string)$icount .'">';
									echo '<option value=""> -- เลือก --</option>';
								while($x = mysqli_fetch_array($orgs)) {
								  echo '<option value="'.$x['code'].'" ';
									if($x['code'] == $r['org_code6']){ echo ' selected="selected" '; }
								  echo '>'.$x['name'].'</option>';
								}
								mysqli_data_seek($orgs,0);
					echo '		</select>
...
$(document).ready(function(){	
	$('a[name=btn_submit_row]').click(function(){
		...
		var params = {
			...
			org_code5: $(this).closest("tr").find('td:eq(5) select option:selected').val(),
			org_code6: $(this).closest("tr").find('td:eq(6) select option:selected').val()
		};
		$.ajax({
